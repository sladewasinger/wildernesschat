...
- `v3.html` + `src/v3/main.ts`
  - Parallel sandbox entrypoint for clean-slate terrain/water prototyping.
  - Isolated from both main and V2 tracks to keep early-stage iteration scoped and maintainable.
- `src/game.ts`
...
- `src/v2/generator/manual-placement.ts`
  - Interactive/manual house placement helpers (road-aware/contour-aware house orientation and contour-following spline roads for click-to-place validation).
...
- `src/v3/config.ts`
  - Centralized V3 constants for view controls, water generation parameters, render palette, and cache limits.
- `src/v3/water-field.ts`
  - Deterministic ocean/river field synthesis with domain-warped noise and explicit water-kind classification.
- `src/v3/terrain-sampler.ts`
  - Cached terrain sampling facade combining grass-tone noise with V3 water masks.
- `src/v3/terrain-renderer.ts`
  - Terrain raster pass that maps cached terrain samples to land/ocean/river colors.
- `src/v3/app.ts`
  - V3 camera/input loop and HUD for terrain-only validation.
