...
### Phase 13: Parallel V3 Sandbox (Fresh Foundation)

- [x] Create isolated `v3.html` + `src/v3/*` prototype track.
- [x] Implement deterministic flat-grass terrain baseline with water-only structure (oceans + rivers).
- [x] Add modular V3 split (`water-field`, `terrain-sampler`, `terrain-renderer`, `app`) with cached terrain sampling.
- [ ] Add tree generation/rendering pass on top of V3 terrain.
- [ ] Add house/road placement pass on top of V3 terrain.
