...
export class V2App {
  ...
  private buildManualConnectionRoad(
    ...
    const searchRadius = hasExistingRoads
      ...
      : previous
        ...
        : 0;
    const facingRoads = hasExistingRoads ? this.manualRoads.filter((road) => road.className === "trunk") : [];
    const terrainHouse = createManualHouseAt(house.id, house.x, house.y, this.terrain, facingRoads);
    if (hasExistingRoads) {
  ...
  private buildManualMainRoadConnection(
    ...
    if (directAttachAnyTrunk && directAttachAnyTrunk.distance <= maxDirectAttachDistance) {
      const alignedHouse = this.alignHouseFacingToAttachment(house, directAttachAnyTrunk);
      const drivewayRoad = this.buildSimpleTDrivewayRoad(id, alignedHouse, directAttachAnyTrunk);
      if (!drivewayRoad) {
        return null;
      }
      if (this.roadIntersectsHouses(drivewayRoad, this.manualHouses, new Set([alignedHouse.id]))) {
        return null;
      }
      return {
        house: alignedHouse,
    ...
    const alignedHouse = this.alignHouseFacingToAttachment(house, drivewayAttach);
    const drivewayRoad = this.buildSimpleTDrivewayRoad(id, alignedHouse, drivewayAttach);
    if (!drivewayRoad) {
      return null;
    }
    if (this.roadIntersectsHouses(drivewayRoad, this.manualHouses, new Set([alignedHouse.id]))) {
  ...
  private alignHouseFacingToAttachment(
    house: House,
    attach: { point: Point; tangentX: number; tangentY: number }
  ): House {
    const tangent = this.normalizeDirection(attach.tangentX, attach.tangentY);
    if (!tangent) {
      return house;
    }
    const front = this.houseFrontPoint(house);
    const toAttach = this.normalizeDirection(attach.point.x - front.x, attach.point.y - front.y);
    const currentFacing = { x: Math.cos(house.angle), y: Math.sin(house.angle) };
    const normalA = { x: -tangent.y, y: tangent.x };
    const normalB = { x: tangent.y, y: -tangent.x };
    const desiredRef = toAttach ?? currentFacing;
    const scoreA = normalA.x * desiredRef.x + normalA.y * desiredRef.y;
    const scoreB = normalB.x * desiredRef.x + normalB.y * desiredRef.y;
    const facing = scoreA >= scoreB ? normalA : normalB;
    const frontOffset = house.width * 0.5 - V2_SETTLEMENT_CONFIG.roads.width * 0.24;
    return {
      ...house,
      x: front.x - facing.x * frontOffset,
      y: front.y - facing.y * frontOffset,
  ...
  private drawTerrain(ctx: CanvasRenderingContext2D, width: number, height: number, viewMinX: number, viewMinY: number): void {
    ...
    if (this.stage === 0 || this.manualPlacementMode) {
      const contourStep = this.manualPlacementMode ? baseStep : worldStep;
      this.drawTerrainContours(ctx, viewMinX, viewMinY, viewMaxX, viewMaxY, contourStep);
    }
  }
